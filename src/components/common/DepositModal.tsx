...
export const DepositModal: React.FC<DepositModalProps> = ({ 
  ...
  const selectCommonItem = (item: any) => {
    if (item && item.id) {
      ...
      setItemDescription(language === 'sw' ? item.name : item.nameEn || item.name);
      setItemUnit(item.unit || 'pieces');
      const convertedValue = convertAmount((item.estimatedValue || 0) * (parseFloat(itemQuantity) || 1));
          ...
          <div className="space-y-4">
            ...
            {depositType === 'items' && (
              <div className="space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (selectedItemData) {
                        ...
                        setItemUnit('pieces'); // Default unit, can be adjusted
                        setItemValue(selectedItemData.value.toString());
                      } else {
                        setItemDescription('');
                        setItemUnit('kg');
                        setItemValue('');
                ...
                {/* Item Quantity and Unit (optional, based on common items) */}
                {selectedItem && (
